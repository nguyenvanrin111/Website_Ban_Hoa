                            ...
                            <tbody>
                                ...
                                <form action="update-quantity">
                                    ...
                                    <input type="hidden" name="productId" value="${C.value.product.id}"/>
                                        ...
                                        <td><img src="${C.value.product.imageURL}" width="50px"/></td>
                                        <td>${C.value.product.price}</td>
                                        <td><input onchange="this.form.submit()" type="number" name="quantity" value="${C.value.quantity}"/></td>
                                        <td>${C.value.product.price * C.value.quantity}</td>
                                        <td><a href="delete-cart?productId=${C.value.product.id}" class="btn btn-outline-danger"><i class="bi bi-bag-x"></i></a></td>
                                    </tr>
                                </form>
                            </c:forEach>
